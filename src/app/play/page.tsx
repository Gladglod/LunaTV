...
function PlayPageClient() {
  ...
  // 处理弹幕
  const handleDanmChange = async (danmuId: number) => {
    if (danmuId <= 0) {
      setDanmuId(0);
      setDanmuUrl('');
      return;
  ...
  useEffect(() => {
    ...
    // 只有非空字符串才加载弹幕
    if (typeof danmuUrl === 'string' && danmuUrl.trim().length > 0) {
      ...
      console.log('重载弹幕：', danmuUrl);
    } else {
      // 清空或禁用弹幕
      artPlayerRef.current.plugins.artplayerPluginDanmuku.config({
        danmuku: '',
      });
      artPlayerRef.current.plugins.artplayerPluginDanmuku.unload?.();
      console.log('清空弹幕');
    }
  }, [danmuUrl]);
          ...
          >
            ...
            >
              <EpisodeSelector
                ...
                onDanmuChange={handleDanmChange}
